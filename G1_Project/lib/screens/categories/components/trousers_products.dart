...
class TrousersProducts extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => _Products();
}

class _Products extends State<TrousersProducts> {
  ...
  Future<void> _fetch() async {
    const url = 'http://192.168.1.6:3000/trousers';
    final response = await http.get(Uri.parse(url));
    if (response.statusCode == 200) {
      final data = json.decode(response.body);
      List<Product> list = [];
      data["trousersList"].forEach((element) {
